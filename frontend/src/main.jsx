...
const ALL_PAGES=[['dashboard','لوحة التحكم',LayoutDashboard],['live_board','العرض الحي',Activity],['records','كشف الصيانة',FileText],['villas','الفلل',Building2],['apartments','الشقق',Home],['technicians','الفنيين',Wrench],['tenants_mgmt','المستأجرين',UserCheck],['leases','الإيجارات',Banknote],['payments_tracker','الدفعات',ListChecks],['inventory','المخزن',Boxes]];
...
function ImportData(){
  ...
  const PREVIEW_COLS={
    ...
    tenants:['الاسم *','الهاتف','رقم الهوية','رقم السيارة','الإيميل'],
...
<div className="aptDetailHeader">
  <div className="aptDetailInfo">
    <div className="aptDetailAvatar"><Home size={24}/></div>
    <div className="aptDetailText">
      <h3 className="aptDetailName">شقة {apt.apartment_no}</h3>
      <div className="aptDetailMeta">
        <span className="aptDetailMetaItem"><Building2 size={13}/>{apt.villa_name}</span>
        {apt.apt_type&&<span className="aptDetailMetaItem">{apt.apt_type}</span>}
        {apt.floor&&<span className="aptDetailMetaItem">الدور: {apt.floor}</span>}
        <span className="aptDetailMetaItem"><Droplet size={12}/>{apt.bathrooms??1} حمام</span>
        {apt.has_balcony?<span className="aptDetailMetaItem">بلكونة</span>:null}
      </div>
    </div>
  </div>
  <div className="aptDetailActions">
    <span className={'aptDetailStatus'+(apt.rental_status==='rented'?' aptDetailStatusRented':' aptDetailStatusAvail')}>{apt.rental_status==='rented'?'مأجورة':'متاحة'}</span>
...
{leasesDetail.map(({lease,installments},idx)=>{
  const st=leaseStatus(lease,installments.filter(i=>i.status==='overdue').length);
  ...
  return <div key={lease.id} className={'leaseBlock'+(st==='expired'?' leaseBlockExpired':'')}>
    <div className="leaseBlockStrip">
      <div className="leaseBlockMeta">
        {idx===0&&st==='active'?<span className="leaseBlockBadge leaseBlockBadgeActive">الحالي</span>:<span className={'leaseBlockBadge leaseBlockBadge'+(st==='under_review'?'Review':'Expired')}>{LEASE_STATUS_LABEL[st]}</span>}
...
<Modal open={payOpen} onClose={()=>setPayOpen(false)} title={`مدفوعات دفعة: ${Number(paymentsInst?.amount||0).toFixed(2)} AED`}>
  {paymentsInst&&<div className="paymentsModal">
    ...
    <form className="form compact" style={{borderTop:'1px solid var(--line)',marginTop:12,paddingTop:12}} onSubmit={addPayment}>
      <Field label="المبلغ المدفوع (AED)" required><div className="payAmountRow"><input required type="number" min="0.01" step="0.01" value={payForm.amount} onChange={e=>setPayForm({...payForm,amount:e.target.value})}/><button type="button" className="payAllBtn" title="ملء المبلغ المتبقّي بالكامل" onClick={()=>{const rem=Math.round((Number(paymentsInst.amount)-payments.reduce((s,p)=>s+Number(p.amount),0))*100)/100;setPayForm({...payForm,amount:String(Math.max(0,rem))})}}><Coins size={14}/>كامل</button></div></Field>
...
const INST_STATUS_LABELS={collected:'تم التحصيل',overdue:'متأخرة',partial:'جزئي',due_soon:'قيد التحصيل',upcoming:'قادمة',cancelled:'مُلغاة',settled:'مُسوّاة',settlement:'تسوية مستحقة'};
const INST_STATUS_CSS={collected:'status-on',overdue:'status-danger',partial:'status-partial',due_soon:'status-warn',upcoming:'status-off',cancelled:'status-cancelled',settled:'status-settled',settlement:'status-settlement'};
...
function leaseStatus(l,overdueCount){const today=new Date().toISOString().slice(0,10);const ended=!l.is_active||l.end_date<today;if(!ended)return 'active';if(l.terminated_at)return 'expired';/* إنهاء يدوي بتسوية → يبقى منتهي */const oc=overdueCount!=null?overdueCount:Number(l.overdue_count||0);return oc>0?'under_review':'expired';}
function InstTable({installments,isAdmin,leaseId,onAdd,onEdit,onDelete,onPayments,onGenerate}){return(
<div className="instTable">
  <div className="instTableHead"><span className="instTableTitle">الدفعات ({installments.length})</span>
    <div className="instTableHeadBtns">
      {onGenerate&&<button className="secondary instGenBtn" onClick={()=>onGenerate(leaseId)}><Zap size={12}/>توليد تلقائي</button>}
      <button className="secondary instTableAdd" onClick={()=>onAdd(leaseId)}><Plus size={12}/>إضافة</button>
    </div>
  </div>
  ...
  <tr key={i.id}>
    ...
    <td className="instTblActions">
      <div className="instTblActionsWrap">
        {i.status==='cancelled'
          ? <span className="instTblCancelledDash">—</span>
          : i.locked
          ? <button className="secondary instHistoryBtn" title="سجل الدفعة (مقفلة)" onClick={()=>onPayments(i)}><Clock size={13}/>السجل</button>
          : <>
...
const LEASE_STATUS_LABEL={active:'فعّال',expired:'منتهي',under_review:'قيد التدقيق'};
const LEASE_STATUS_CSS={active:'status-on',expired:'status-off',under_review:'status-review'};
...
const empty={name:'',phone:'',national_id:'',car_number:'',email:'',notes:''};
...
const filtered=rows.filter(r=>!qs||r.name.includes(qs)||(r.phone||'').includes(qs)||(r.national_id||'').includes(qs)||(r.car_number||'').includes(qs));
...
if(selected){
      ...
      <div className="tenantProfileMain">
        ...
        <div className="tenantProfileInfo">
          ...
          <div className="tenantProfileContacts">
            ...
            {tenant.car_number&&<span className="tenantProfileContact"><Car size={13}/>{tenant.car_number}</span>}
            {tenant.email&&<span className="tenantProfileContact"><Mail size={13}/>{tenant.email}</span>}
          </div>
        </div>
        <div className="tenantProfileActions">
          <button onClick={()=>setLeaseOpen(true)}><Plus size={15}/>عقد جديد</button>
          <button className="secondary" onClick={()=>{setEditing(tenant.id);setForm({name:tenant.name,phone:tenant.phone||'',national_id:tenant.national_id||'',car_number:tenant.car_number||'',email:tenant.email||'',notes:tenant.notes||''});setTenantOpen(true)}}><Edit size={15}/>تعديل</button>
  ...
  <div className="tenantLeasesSection">
    ...
    sorted.map(({lease,installments},idx)=>{
      ...
      return <div key={lease.id} className={'leaseBlock'+(st==='expired'?' leaseBlockExpired':'')}>
        <div className="leaseBlockStrip">
          <div className="leaseBlockMeta">
            {st==='active'?<span className="leaseBlockBadge leaseBlockBadgeActive">نشط</span>:<span className={'leaseBlockBadge leaseBlockBadge'+(st==='under_review'?'Review':'Expired')}>{LEASE_STATUS_LABEL[st]}</span>}
  ...
  <Modal open={payOpen} onClose={()=>setPayOpen(false)} title={paymentsInst?.locked?`سجل الدفعة: ${Number(paymentsInst?.amount||0).toFixed(2)} AED`:`مدفوعات دفعة: ${Number(paymentsInst?.amount||0).toFixed(2)} AED`}>
    {paymentsInst&&<div className="paymentsModal">
      {paymentsInst.locked&&<div className="payLockedNote"><Lock size={13}/>هذه الدفعة مُحصّلة ومقفلة — سجل للقراءة فقط.</div>}
      <div className="paymentsList">{payments.length===0&&<p className="empty" style={{padding:12,textAlign:'center'}}>لا توجد مدفوعات بعد</p>}{payments.map(p=><div key={p.id} className="paymentRow"><div className="payRowMain"><span className="payAmount">{Number(p.amount).toFixed(2)} AED</span><span className="payDate"><Calendar size={11}/>{new Date(p.payment_date).toLocaleDateString('ar-AE')}</span>{p.created_by_name&&<span className="payBy"><UserCheck size={11}/>{p.created_by_name}</span>}{p.created_at&&<span className="payStamp"><Clock size={11}/>{new Date(String(p.created_at).replace(' ','T')).toLocaleString('ar-AE',{dateStyle:'short',timeStyle:'short'})}</span>}{p.notes&&<span className="payNotes">{p.notes}</span>}</div>{isAdmin&&!paymentsInst.locked&&<button className="danger iconBtn" onClick={()=>removePayment(p.id)}><Trash2 size={14}/></button>}</div>)}</div>
      {!paymentsInst.locked&&<form className="form compact" style={{borderTop:'1px solid var(--line)',marginTop:12,paddingTop:12}} onSubmit={addPayment}>
        <Field label="المبلغ المدفوع (AED)" required><div className="payAmountRow"><input required type="number" min="0.01" step="0.01" value={payForm.amount} onChange={e=>setPayForm({...payForm,amount:e.target.value})}/><button type="button" className="payAllBtn" title="ملء المبلغ المتبقّي بالكامل" onClick={()=>{const rem=Math.round((Number(paymentsInst.amount)-payments.reduce((s,p)=>s+Number(p.amount),0))*100)/100;setPayForm({...payForm,amount:String(Math.max(0,rem))})}}><Coins size={14}/>كامل</button></div></Field>
        <Field label="تاريخ الدفع" required><input required type="date" value={payForm.payment_date} onChange={e=>setPayForm({...payForm,payment_date:e.target.value})}/></Field>
        <Field label="ملاحظات" wide><input value={payForm.notes} onChange={e=>setPayForm({...payForm,notes:e.target.value})}/></Field>
        <button><Plus size={14}/>تسجيل دفع</button>
  ...
  <Modal open={tenantOpen} onClose={()=>setTenantOpen(false)} title={editing?'تعديل مستأجر':'إضافة مستأجر'}><form className="form compact" onSubmit={saveTenant}>
    ...
    <Field label="رقم السيارة"><input value={form.car_number} onChange={e=>setForm({...form,car_number:e.target.value})} placeholder="أ ب ج 12345"/></Field>
...
{filtered.map((r,i)=><div key={r.id} className="tenantRow" onClick={()=>openTenant(r)}>
  ...
  <div className="tenantRowMain">
    ...
    <div className="tenantRowMeta">
      ...
      {r.car_number&&<span className="tenantRowMetaItem"><Car size={11}/>{r.car_number}</span>}
  ...
  <div className="tenantRowActions" onClick={e=>e.stopPropagation()}>
    ...
    <button className="iconBtn secondary" onClick={e=>{e.stopPropagation();setEditing(r.id);setForm({name:r.name,phone:r.phone||'',national_id:r.national_id||'',car_number:r.car_number||'',email:r.email||'',notes:r.notes||''});setTenantOpen(true)}}><Edit size={13}/></button>
...
const[form,setForm]=useState(empty);const[editing,setEditing]=useState(null);const[open,setOpen]=useState(false);const[renewing,setRenewing]=useState(false);
...
const[genOpen,setGenOpen]=useState(false);const[genFreq,setGenFreq]=useState('quarterly');const[genBusy,setGenBusy]=useState(false);
const[paymentsInst,setPaymentsInst]=useState(null);const[payments,setPayments]=useState([]);
const[payForm,setPayForm]=useState({amount:'',payment_date:new Date().toISOString().slice(0,10),notes:''});const[payOpen,setPayOpen]=useState(false);
...
async function generateInstallments(){const hasExisting=(leaseDetail?.installments||[]).length>0;if(hasExisting&&!window.confirm('يوجد دفعات حالية — سيتم حذفها واستبدالها بالدفعات المُولّدة. متابعة؟'))return;setGenBusy(true);try{const r=await api('/leases/'+selectedLease+'/generate-installments',{method:'POST',body:JSON.stringify({frequency:genFreq,replace:hasExisting})});setGenOpen(false);loadDetail(selectedLease);showToast(`تم توليد ${r.generated} دفعة تلقائياً`,'success')}catch(e){showToast(e.message||'تعذّر التوليد','error')}finally{setGenBusy(false)}}

async function saveLease(e){e.preventDefault();const wasRenewing=renewing;await runAction(async()=>{const{_villa_id,...rest}=form;const res=await api(editing?'/leases/'+editing:'/leases',{method:editing?'PUT':'POST',body:JSON.stringify(rest)});setForm(empty);setEditing(null);setRenewing(false);setOpen(false);load();if(wasRenewing&&res?.id){loadDetail(res.id)}else if(selectedLease){loadDetail(selectedLease)}},editing?'تم حفظ التعديل':wasRenewing?'تم تجديد العقد بنجاح':'تمت إضافة العقد')}
function renewLease(lease){const endD=String(lease.end_date).slice(0,10);const ns=new Date(endD);const ne=new Date(endD);ne.setFullYear(ne.getFullYear()+1);const toISO=d=>d.toISOString().slice(0,10);setEditing(null);setRenewing(true);setForm({apartment_id:lease.apartment_id,tenant_id:lease.tenant_id,start_date:toISO(ns),end_date:toISO(ne),total_amount:lease.total_amount,fees_amount:lease.fees_amount||'',deposit_amount:lease.deposit_amount||'',deposit_type:lease.deposit_type||'',deposit_notes:lease.deposit_notes||'',notes:'',is_active:1,_villa_id:''});setOpen(true)}
...
const activeCount=rows.filter(r=>leaseStatus(r)==='active').length;
const reviewCount=rows.filter(r=>leaseStatus(r)==='under_review').length;

// ─── Detail view ───────────────────────────────────────────────
if(selectedLease&&leaseDetail){
  const{lease,installments}=leaseDetail;
  const overdueCount=installments.filter(i=>i.status==='overdue').length;
  const st=leaseStatus(lease,overdueCount);
  const collected=installments.reduce((s,i)=>s+Number(i.collected_amount),0);
  const remaining=Number(lease.total_amount)-collected;
  ...
  <div className="leaseDetailPage">
    ...
    <div className="ldCard">
      ...
      <div className="ldCardTop">
        <div className="ldCardLeft">
          <div className="ldCardBadges">
            <span className={'leaseBlockBadge leaseBlockBadge'+(st==='active'?'Active':st==='under_review'?'Review':'Expired')}>{st==='active'?'نشط':LEASE_STATUS_LABEL[st]}</span>
        ...
        <div className="ldCardActions">
          ...
          {lease.terminated_at
            ...
            : st==='active'&&isAdmin&&<button className="secondary ldTerminateBtn" onClick={()=>{setTerminateDate('');setTerminateOpen(true)}}><X size={14}/>إنهاء العقد</button>}
          {st==='expired'&&isAdmin&&<button className="secondary ldRenewBtn" onClick={()=>renewLease(lease)}><RefreshCw size={14}/>تجديد العقد</button>}
      ...
      <div className="ldProgressWrap">
        <div className="ldProgressTrack"><div className="ldProgressFill" style={{width:pct+'%',background:st==='under_review'?'linear-gradient(90deg,#d97706,#f59e0b)':st==='expired'?'#94a3b8':'linear-gradient(90deg,#0f766e,#0e7490)'}}/></div>
        <span className="ldProgressLabel">{pct}% مكتمل</span>
      </div>
      {/* Installments — merged into the same card */}
      <InstTable installments={installments} isAdmin={isAdmin} leaseId={lease.id}
        ...
        onDelete={removeInst} onPayments={openPayments}
        onGenerate={()=>{setGenFreq('quarterly');setGenOpen(true)}}/>
    </div>
  </div>

  <Modal open={open} onClose={()=>{setOpen(false);setRenewing(false)}} title={renewing?'تجديد العقد':'تعديل عقد'}><form className="form" onSubmit={saveLease}>
    {renewing&&<div className="renewHint"><RefreshCw size={13}/>عقد جديد بنفس المستأجر والشقة والتأمين — عدّل قيمة الإيجار والتواريخ.</div>}
    <LeaseFormFields form={form} setForm={setForm} tenants={tenants}/>
    {!renewing&&<Field label="الحالة"><select value={form.is_active} onChange={e=>setForm({...form,is_active:Number(e.target.value)})}><option value={1}>فعّال</option><option value={0}>منتهي</option></select></Field>}
    <button><Plus size={16}/>{renewing?'إنشاء العقد الجديد':'حفظ التعديل'}</button><button type="button" className="secondary" onClick={()=>{setOpen(false);setRenewing(false)}}>إلغاء</button>
  </form></Modal>
  <Modal open={instOpen} onClose={()=>setInstOpen(false)} title={editingInst?'تعديل دفعة':'إضافة دفعة'}><form className="form compact" onSubmit={saveInst}>
    <Field label="تاريخ الاستحقاق" required><input required type="date" value={instForm.due_date} onChange={e=>setInstForm({...instForm,due_date:e.target.value})}/></Field>
    <Field label="المبلغ (AED)" required><input required type="number" min="0.01" step="0.01" value={instForm.amount} onChange={e=>setInstForm({...instForm,amount:e.target.value})}/></Field>
  ...
  {/* ── Auto-generate installments modal ── */}
  <Modal open={genOpen} onClose={()=>setGenOpen(false)} title="توليد دفعات تلقائياً">
    {leaseDetail&&(()=>{
      const addM=(s,n)=>{const[y,m,d]=s.split('-').map(Number);const t=(m-1)+n;const ny=y+Math.floor(t/12);const nm=((t%12)+12)%12;const ld=new Date(Date.UTC(ny,nm+1,0)).getUTCDate();return `${ny}-${String(nm+1).padStart(2,'0')}-${String(Math.min(d,ld)).padStart(2,'0')}`};
      const start=String(leaseDetail.lease.start_date).slice(0,10),end=String(leaseDetail.lease.end_date).slice(0,10);
      const total=Number(leaseDetail.lease.total_amount);
      const interval={monthly:1,quarterly:3,semiannual:6,annual:12}[genFreq];
      const dues=[];for(let k=0;k<600;k++){const d=addM(start,k*interval);if(d>=end)break;dues.push(d)}if(!dues.length)dues.push(start);
      const per=Math.floor((total/dues.length)*100)/100;
      const hasExisting=(leaseDetail.installments||[]).length>0;
      const FREQS=[['monthly','شهري','كل شهر'],['quarterly','ربع سنوي','كل 3 أشهر'],['semiannual','نصف سنوي','كل 6 أشهر'],['annual','سنوي','مرة واحدة']];
      return <div className="genModal">
        <div className="genField">إجمالي العقد: <strong>{total.toLocaleString()} AED</strong> · من {new Date(start).toLocaleDateString('ar-AE')} إلى {new Date(end).toLocaleDateString('ar-AE')}</div>
        <div className="genFreqGrid">
          {FREQS.map(([v,l,sub])=><button key={v} type="button" className={'genFreqCard'+(genFreq===v?' genFreqActive':'')} onClick={()=>setGenFreq(v)}>
            <span className="genFreqLbl">{l}</span><span className="genFreqSub">{sub}</span>
          </button>)}
        </div>
        <div className="genPreview">
          <div className="genPreviewRow"><span>عدد الدفعات</span><strong>{dues.length}</strong></div>
          <div className="genPreviewRow"><span>قيمة كل دفعة</span><strong>{per.toLocaleString()} AED</strong></div>
          <div className="genPreviewRow"><span>أول دفعة</span><strong>{new Date(dues[0]).toLocaleDateString('ar-AE')}</strong></div>
        </div>
        {hasExisting&&<div className="genWarn"><AlertCircle size={13}/>يوجد {leaseDetail.installments.length} دفعة حالية — سيتم حذفها واستبدالها.</div>}
        <button type="button" className="genConfirmBtn" disabled={genBusy} onClick={generateInstallments}><Zap size={15}/>{genBusy?'جاري التوليد...':`توليد ${dues.length} دفعة`}</button>
      </div>;
    })()}
  </Modal>
  <Modal open={payOpen} onClose={()=>setPayOpen(false)} title={paymentsInst?.locked?`سجل الدفعة: ${Number(paymentsInst?.amount||0).toLocaleString()} AED`:`مدفوعات: ${Number(paymentsInst?.amount||0).toLocaleString()} AED`}>
    {paymentsInst&&<div className="paymentsModal">
      {paymentsInst.locked&&<div className="payLockedNote"><Lock size={13}/>هذه الدفعة مُحصّلة ومقفلة — سجل للقراءة فقط.</div>}
      <div className="paymentsList">{payments.length===0&&<p className="empty" style={{padding:12,textAlign:'center'}}>لا توجد مدفوعات بعد</p>}{payments.map(p=><div key={p.id} className="paymentRow"><div className="payRowMain"><span className="payAmount">{Number(p.amount).toLocaleString()} AED</span><span className="payDate"><Calendar size={11}/>{new Date(p.payment_date).toLocaleDateString('ar-AE')}</span>{p.created_by_name&&<span className="payBy"><UserCheck size={11}/>{p.created_by_name}</span>}{p.created_at&&<span className="payStamp"><Clock size={11}/>{new Date(String(p.created_at).replace(' ','T')).toLocaleString('ar-AE',{dateStyle:'short',timeStyle:'short'})}</span>}{p.notes&&<span className="payNotes">{p.notes}</span>}</div>{isAdmin&&!paymentsInst.locked&&<button className="danger iconBtn" onClick={()=>removePayment(p.id)}><Trash2 size={14}/></button>}</div>)}</div>
      {!paymentsInst.locked&&<form className="form compact" style={{borderTop:'1px solid var(--line)',marginTop:12,paddingTop:12}} onSubmit={addPayment}>
        <Field label="المبلغ المدفوع (AED)" required><div className="payAmountRow"><input required type="number" min="0.01" step="0.01" value={payForm.amount} onChange={e=>setPayForm({...payForm,amount:e.target.value})}/><button type="button" className="payAllBtn" title="ملء المبلغ المتبقّي بالكامل" onClick={()=>{const rem=Math.round((Number(paymentsInst.amount)-payments.reduce((s,p)=>s+Number(p.amount),0))*100)/100;setPayForm({...payForm,amount:String(Math.max(0,rem))})}}><Coins size={14}/>كامل</button></div></Field>
        <Field label="تاريخ الدفع" required><input required type="date" value={payForm.payment_date} onChange={e=>setPayForm({...payForm,payment_date:e.target.value})}/></Field>
        <Field label="ملاحظات" wide><input value={payForm.notes} onChange={e=>setPayForm({...payForm,notes:e.target.value})}/></Field>
  ...
  {(()=>{
    ...
    const rentDebt=tenantOwes?Math.abs(balance):0;
    ...
    // final amount the tenant owes the company after everything
    const finalOwedByTenant=depositAmt>0?Math.max(0,-(depositAfterAll)):(rentDebt+totalExtra);
    ...
    return <Modal open={terminateOpen} onClose={()=>{setTerminateOpen(false);setExtraDeductions([]);setExtraDesc('');setExtraAmt('');}} title="إنهاء العقد مبكراً">
      <div className="terminateModal">
        ...
        {tDate&&<>
          ...
          {/* Additional charges owed to the company — always available */}
          <div className="terminateChargesSection">
            <div className="terminateChargesHeader"><Coins size={14}/>مبالغ إضافية مستحقة للشركة <span className="terminateChargesOpt">(اختياري)</span></div>
            {extraDeductions.map((d,i)=><div key={i} className="terminateChargeRow">
              <span className="terminateChargeDesc">{d.desc}</span>
              <span className="terminateChargeAmt">{Number(d.amt).toLocaleString()} AED</span>
            ...
            <div className="terminateAddDeduction">
              <input className="terminateAddDesc" placeholder="سبب المبلغ (تلف، تنظيف، فاتورة كهرباء...)" value={extraDesc} onChange={e=>setExtraDesc(e.target.value)}/>
            ...
            {totalExtra>0&&<div className="terminateChargesTotal"><span>إجمالي المبالغ المستحقة للشركة</span><strong>{totalExtra.toLocaleString()} AED</strong></div>}
          </div>
          {/* Deposit section (cash) — absorbs rent balance + company charges */}
          {depositAmt>0&&<div className="terminateDepositSection">
            ...
            {totalExtra>0&&<div className="terminateDepositLine">
              <span>مبالغ مستحقة للشركة</span>
              <span style={{color:'#dc2626'}}>−{totalExtra.toLocaleString()} AED</span>
            </div>}
          ...
          {/* Final amount owed to company (no cash deposit to absorb it) */}
          {depositAmt<=0&&finalOwedByTenant>0&&<div className="terminateResult terminateResultOwes">
            <div className="terminateResultIcon">🏢</div>
            <div style={{flex:1}}>
              <div className="terminateResultTitle">المستحق على المستأجر للشركة</div>
              <div className="terminateResultAmt">{finalOwedByTenant.toLocaleString()} AED</div>
              <div className="terminateResultSub">{[rentDebt>0?`فرق إيجار ${rentDebt.toLocaleString()}`:null,totalExtra>0?`مبالغ إضافية ${totalExtra.toLocaleString()}`:null].filter(Boolean).join(' + ')}</div>
            </div>
          </div>}
        ...
        {tDate&&<button type="button" style={{width:'100%',marginTop:4,background:'#0369a1',color:'#fff',border:'none',borderRadius:9,padding:'10px 0',fontWeight:700,fontSize:13,display:'flex',alignItems:'center',justifyContent:'center',gap:7,cursor:'pointer'}} onClick={()=>{
          ...
          `:'';
          const chargesRows=totalExtra>0?`
            <tr style="background:#fffbeb"><td colspan="2" style="font-weight:700;color:#b45309;padding:9px 12px">مبالغ إضافية مستحقة للشركة</td></tr>
            ${extraDeductions.map(d=>`<tr><td>${d.desc}</td><td style="color:#b45309">${Number(d.amt).toLocaleString()} AED</td></tr>`).join('')}
          `:'';
          const owedRow=(depositAmt<=0&&finalOwedByTenant>0)?`<tr style="background:#fef2f2"><td style="font-weight:700">المستحق على المستأجر للشركة</td><td style="font-weight:900;font-size:16px;color:#dc2626">${finalOwedByTenant.toLocaleString()} AED</td></tr>`:'';
          ...
          ${balanceRow}${chargesRows}${depRows}${owedRow}
          </tbody></table>
          ${unpaidHtml}
        ...
        {tDate&&<button type="button" style={{width:'100%',marginTop:4,background:'#b91c1c',color:'#fff',border:'none',borderRadius:9,padding:'11px 0',fontWeight:700,fontSize:13,display:'flex',alignItems:'center',justifyContent:'center',gap:7,cursor:'pointer'}} onClick={async()=>{
          ...
          // deposit (cash only) absorbs rent debt + extra charges; remainder owed becomes an installment.
          // without a cash deposit, extra company charges add directly to what the tenant owes.
          const depositObligations=rentDebt+totalExtra;
          const newOwed=depositAmt>0?Math.max(0,depositObligations-depositAmt):rentDebt+totalExtra;
          ...
          if(totalExtra>0){L.push(`مبالغ إضافية مستحقة للشركة: ${totalExtra.toLocaleString()} AED`);extraDeductions.forEach(d=>L.push(`  - ${d.desc}: ${Number(d.amt).toLocaleString()} AED`));}
...
<div className="leasesHeader">
  ...
  <div className="leasesToolbar">
    <div className="leasesFilterTabs">
      {[['all','الكل',rows.length],['active','نشط',activeCount],['under_review','قيد التدقيق',reviewCount],['expired','منتهي',rows.length-activeCount-reviewCount]].map(([v,l,c])=>
        <button key={v} className={'leasesFilterTab'+(statusFilter===v?' leasesFilterTabActive':'')+(v==='under_review'&&c>0?' leasesFilterTabReview':'')} onClick={()=>setStatusFilter(v)}>{l}<span className="leasesFilterCount">{c}</span></button>
      )}
    </div>
  ...
  return list.map(g=>{
    ...
    return <div key={g.key} className={'villaSection'+(isOpen?' villaSectionOpen':'')}>
      ...
      {g.leases.map(r=>{
        ...
        return <div key={r.id} className={'leaseRow'+(st==='expired'?' leaseRowExpired':'')+(st==='under_review'?' leaseRowReview':'')} onClick={()=>loadDetail(r.id)}>
          ...
          <div className="leaseRowProgress">
            <div className="leaseRowProgressBar"><div className="leaseRowProgressFill" style={{width:pct+'%',background:st==='under_review'?'#f59e0b':st==='expired'?'#94a3b8':undefined}}/></div>
...
const INV_UNITS=['قطعة','متر','كيلو','لتر','علبة','لفة','كيس','عبوة','طقم'];
const INV_STATUS={ok:{l:'متوفر',c:'status-on'},low:{l:'منخفض',c:'status-warn'},out:{l:'نفد',c:'status-danger'}};
const MOV_TYPE={purchase:{l:'شراء',c:'movPurchase',icon:'+'},consume:{l:'صرف',c:'movConsume',icon:'−'},adjust:{l:'تسوية',c:'movAdjust',icon:'±'}};
...
function onImpFile(e){const f=e.target.files[0];if(!f)return;setImpFile(f);impLoadPreview(f)}
async function impConfirm(){if(!impFile)return;setImpLoading(true);try{const fd=new FormData();fd.append('file',impFile);const r=await fetch(API_BASE()+'/inventory/import/confirm',{method:'POST',headers:{Authorization:'Bearer '+localStorage.token},body:fd});const j=await r.json();if(!r.ok)throw new Error(j.message||'خطأ');setImpResult(j.data);setImpPreview(null);setImpFile(null);loadAll();showToast('تم استيراد بيانات المخزن','success')}catch(e){showToast(e.message,'error')}setImpLoading(false)}
useEffect(()=>{loadAll()},[]);
const fmt=n=>Number(n||0).toLocaleString('en-US',{maximumFractionDigits:2});

async function saveItem(e){e.preventDefault();await runAction(async()=>{await api(editingItem?'/inventory/items/'+editingItem:'/inventory/items',{method:editingItem?'PUT':'POST',body:JSON.stringify(itemForm)});setItemForm(emptyItem);setEditingItem(null);setItemOpen(false);loadAll()},editingItem?'تم حفظ الصنف':'تمت إضافة الصنف')}
async function removeItem(r){if(!confirm('حذف الصنف؟ سيُحذف سجل حركاته أيضاً.'))return;await runAction(async()=>{await api('/inventory/items/'+r.id,{method:'DELETE'});loadAll()},'تم الحذف')}
async function saveSup(e){e.preventDefault();await runAction(async()=>{await api(editingSup?'/suppliers/'+editingSup:'/suppliers',{method:editingSup?'PUT':'POST',body:JSON.stringify(supForm)});setSupForm(emptySup);setEditingSup(null);setSupOpen(false);loadAll()},editingSup?'تم حفظ المورّد':'تمت إضافة المورّد')}
async function removeSup(r){if(!confirm('حذف المورّد؟'))return;await runAction(async()=>{await api('/suppliers/'+r.id,{method:'DELETE'});loadAll()},'تم الحذف')}
function openMov(type,itemId=''){setMovType(type);setMovForm({...emptyMov,item_id:itemId});setMovOpen(true)}
async function saveMov(e){e.preventDefault();await runAction(async()=>{await api('/inventory/movements',{method:'POST',body:JSON.stringify({...movForm,type:movType})});setMovForm(emptyMov);setMovOpen(false);loadAll()},movType==='purchase'?'تم تسجيل الشراء':movType==='consume'?'تم تسجيل الصرف':'تمت التسوية')}
async function removeMov(id){if(!confirm('حذف الحركة؟ سيتأثر رصيد المخزون.'))return;await runAction(async()=>{await api('/inventory/movements/'+id,{method:'DELETE'});loadAll()},'تم الحذف')}

if(loading||!summary)return <Loader/>;
const filteredMovs=movFilter?movements.filter(m=>m.type===movFilter):movements;
const selItem=items.find(i=>String(i.id)===String(movForm.item_id));

return <div className="invRoot">
  <div className="invHeader">
    <div className="invHeaderTitle"><Warehouse size={20}/>المخزن</div>
    <div className="invHeaderActions">
      <button onClick={()=>openMov('purchase')}><PackagePlus size={15}/>شراء</button>
      <button className="secondary" onClick={()=>openMov('consume')}><PackageMinus size={15}/>صرف</button>
      {isAdmin&&<button className="secondary" onClick={()=>{setImpPreview(null);setImpResult(null);setImpFile(null);setImpOpen(true)}}><Upload size={15}/>استيراد</button>}
    </div>
  </div>

  {/* KPIs */}
  <div className="invKpiRow">
    <div className="invKpi invKpi-blue"><div className="invKpiIcon"><Boxes size={18}/></div><div><div className="invKpiVal">{summary.total_items}</div><div className="invKpiLabel">عدد الأصناف</div></div></div>
    <div className="invKpi invKpi-green"><div className="invKpiIcon"><Coins size={18}/></div><div><div className="invKpiVal">{fmt(summary.total_value)}</div><div className="invKpiLabel">قيمة المخزون AED</div></div></div>
    <div className="invKpi invKpi-amber"><div className="invKpiIcon"><AlertCircle size={18}/></div><div><div className="invKpiVal">{summary.low_count}</div><div className="invKpiLabel">أصناف منخفضة</div></div></div>
    <div className="invKpi invKpi-red"><div className="invKpiIcon"><Package size={18}/></div><div><div className="invKpiVal">{summary.out_count}</div><div className="invKpiLabel">أصناف نفدت</div></div></div>
    <div className="invKpi invKpi-teal"><div className="invKpiIcon"><PackagePlus size={18}/></div><div><div className="invKpiVal">{fmt(summary.month_purchases.value)}</div><div className="invKpiLabel">مشتريات الشهر AED</div></div></div>
    <div className="invKpi invKpi-purple"><div className="invKpiIcon"><PackageMinus size={18}/></div><div><div className="invKpiVal">{fmt(summary.month_consumption.value)}</div><div className="invKpiLabel">استهلاك الشهر AED</div></div></div>
  </div>

  {/* low stock alert */}
  {summary.low_items.length>0&&<div className="invLowAlert"><AlertCircle size={14}/><span>أصناف وصلت حد إعادة الطلب: {summary.low_items.map(i=>`${i.name} (${i.stock} ${i.unit})`).join(' · ')}</span></div>}

  {/* tabs */}
  <div className="invTabs">
    {[['items','الأصناف',Boxes],['movements','حركات المخزون',ListChecks],['suppliers','الموردون',Truck],['reports','تقارير',BarChart2]].map(([v,l,I])=>
      <button key={v} className={'invTab'+(tab===v?' invTabActive':'')} onClick={()=>setTab(v)}><I size={14}/>{l}</button>
    )}
  </div>

  {/* ITEMS */}
  {tab==='items'&&<div className="invCard">
    <div className="invCardHead"><span className="invCardTitle">الأصناف ({items.length})</span><button className="secondary" onClick={()=>{setEditingItem(null);setItemForm(emptyItem);setItemOpen(true)}}><Plus size={14}/>صنف جديد</button></div>
    <div className="invTableWrap"><table className="invTable">
      <thead><tr><th>الصنف</th><th>الفئة</th><th>المخزون</th><th>متوسط السعر</th><th>القيمة</th><th>حد الطلب</th><th>الحالة</th><th></th></tr></thead>
      <tbody>{items.length===0?<tr><td colSpan={8} className="invEmpty">لا توجد أصناف — اضغط "صنف جديد"</td></tr>:items.map(i=><tr key={i.id} className={i.status==='out'?'invRowOut':i.status==='low'?'invRowLow':''}>
        <td className="invItemName">{i.name}</td>
        <td>{i.category||'—'}</td>
        <td className="invStockCell"><strong>{fmt(i.stock)}</strong> <span className="invUnit">{i.unit}</span></td>
        <td>{fmt(i.avg_cost)}</td>
        <td className="invValCell">{fmt(i.stock_value)}</td>
        <td>{fmt(i.reorder_level)}</td>
        <td><span className={'statusBadge statusBadgeSm '+INV_STATUS[i.status].c}>{INV_STATUS[i.status].l}</span></td>
        <td className="invActions">
          <button className="secondary iconBtn" title="شراء" onClick={()=>openMov('purchase',i.id)}><PackagePlus size={13}/></button>
          <button className="secondary iconBtn" title="صرف" onClick={()=>openMov('consume',i.id)}><PackageMinus size={13}/></button>
          <button className="secondary iconBtn" onClick={()=>{setEditingItem(i.id);setItemForm({name:i.name,unit:i.unit,category:i.category||'',reorder_level:i.reorder_level,notes:i.notes||''});setItemOpen(true)}}><Edit size={13}/></button>
          {isAdmin&&<button className="danger iconBtn" onClick={()=>removeItem(i)}><Trash2 size={13}/></button>}
        </td>
      </tr>)}</tbody>
    </table></div>
  </div>}
  ...
  {/* MOVEMENTS */}
  {tab==='movements'&&<div className="invCard">
    <div className="invCardHead">
      <span className="invCardTitle">حركات المخزون ({filteredMovs.length})</span>
      <div className="invMovFilters">{[['','الكل'],['purchase','شراء'],['consume','صرف'],['adjust','تسوية']].map(([v,l])=><button key={v} className={'invFilterBtn'+(movFilter===v?' invFilterActive':'')} onClick={()=>setMovFilter(v)}>{l}</button>)}</div>
    </div>
    <div className="invTableWrap"><table className="invTable">
      <thead><tr><th>التاريخ</th><th>الصنف</th><th>النوع</th><th>الكمية</th><th>السعر</th><th>الإجمالي</th><th>المورّد/الفيلا</th><th>بواسطة</th><th></th></tr></thead>
      <tbody>{filteredMovs.length===0?<tr><td colSpan={9} className="invEmpty">لا توجد حركات</td></tr>:filteredMovs.map(m=><tr key={m.id}>
        <td>{new Date(m.movement_date).toLocaleDateString('ar-AE')}</td>
        <td className="invItemName">{m.item_name}</td>
        <td><span className={'movBadge '+MOV_TYPE[m.type].c}>{MOV_TYPE[m.type].icon} {MOV_TYPE[m.type].l}</span></td>
  ...
  {/* SUPPLIERS */}
  {tab==='suppliers'&&<div className="invCard">
    <div className="invCardHead"><span className="invCardTitle">الموردون ({suppliers.length})</span><button className="secondary" onClick={()=>{setEditingSup(null);setSupForm(emptySup);setSupOpen(true)}}><Plus size={14}/>مورّد جديد</button></div>
    <div className="invTableWrap"><table className="invTable">
      <thead><tr><th>الاسم</th><th>الهاتف</th><th>ملاحظات</th><th></th></tr></thead>
      <tbody>{suppliers.length===0?<tr><td colSpan={4} className="invEmpty">لا يوجد موردون</td></tr>:suppliers.map(s=><tr key={s.id}>
        <td className="invItemName">{s.name}</td><td>{s.phone||'—'}</td><td>{s.notes||'—'}</td>
        <td className="invActions"><button className="secondary iconBtn" onClick={()=>{setEditingSup(s.id);setSupForm({name:s.name,phone:s.phone||'',notes:s.notes||''});setSupOpen(true)}}><Edit size={13}/></button>{isAdmin&&<button className="danger iconBtn" onClick={()=>removeSup(s)}><Trash2 size={13}/></button>}</td>
      </tr>)}</tbody>
    </table></div>
  </div>}

  {/* REPORTS */}
  {tab==='reports'&&<div className="invReportsGrid">
    <div className="invCard"><div className="invCardHead"><span className="invCardTitle"><PackageMinus size={14}/> الأكثر استهلاكاً</span></div>
      <div className="invTableWrap"><table className="invTable"><thead><tr><th>الصنف</th><th>الكمية</th><th>القيمة</th></tr></thead>
      <tbody>{summary.top_consumed.length===0?<tr><td colSpan={3} className="invEmpty">لا يوجد استهلاك</td></tr>:summary.top_consumed.map((t,i)=><tr key={i}><td className="invItemName">{t.name}</td><td>{fmt(t.qty)} {t.unit}</td><td className="invValCell">{fmt(t.value)}</td></tr>)}</tbody></table></div>
    </div>
    <div className="invCard"><div className="invCardHead"><span className="invCardTitle"><Building2 size={14}/> الاستهلاك حسب الفيلا</span></div>
      <div className="invTableWrap"><table className="invTable"><thead><tr><th>الفيلا</th><th>الكمية</th><th>القيمة</th></tr></thead>
      <tbody>{summary.by_villa.length===0?<tr><td colSpan={3} className="invEmpty">لا يوجد صرف مرتبط بفيلا</td></tr>:summary.by_villa.map((t,i)=><tr key={i}><td className="invItemName">{t.villa_name}</td><td>{fmt(t.qty)}</td><td className="invValCell">{fmt(t.value)}</td></tr>)}</tbody></table></div>
    </div>
    <div className="invCard"><div className="invCardHead"><span className="invCardTitle"><AlertCircle size={14}/> أصناف تحت حد الطلب</span></div>
      <div className="invTableWrap"><table className="invTable"><thead><tr><th>الصنف</th><th>المخزون</th><th>حد الطلب</th></tr></thead>
      <tbody>{summary.low_items.length===0?<tr><td colSpan={3} className="invEmpty">كل الأصناف فوق الحد ✓</td></tr>:summary.low_items.map((t,i)=><tr key={i} className="invRowLow"><td className="invItemName">{t.name}</td><td><strong>{fmt(t.stock)}</strong> {t.unit}</td><td>{fmt(t.reorder_level)}</td></tr>)}</tbody></table></div>
    </div>
  </div>}

  {/* Item modal */}
  <Modal open={itemOpen} onClose={()=>setItemOpen(false)} title={editingItem?'تعديل صنف':'صنف جديد'}><form className="form compact" onSubmit={saveItem}>
    <Field label="اسم الصنف" required><input required value={itemForm.name} onChange={e=>setItemForm({...itemForm,name:e.target.value})}/></Field>
    <Field label="الوحدة"><select value={itemForm.unit} onChange={e=>setItemForm({...itemForm,unit:e.target.value})}>{INV_UNITS.map(u=><option key={u} value={u}>{u}</option>)}</select></Field>
    <Field label="الفئة"><input value={itemForm.category} onChange={e=>setItemForm({...itemForm,category:e.target.value})} placeholder="سباكة، كهرباء، تكييف..."/></Field>
    <Field label="حد إعادة الطلب"><input type="number" min="0" step="0.01" value={itemForm.reorder_level} onChange={e=>setItemForm({...itemForm,reorder_level:e.target.value})}/></Field>
    <Field label="ملاحظات" wide><textarea value={itemForm.notes} onChange={e=>setItemForm({...itemForm,notes:e.target.value})}/></Field>
    <button>{editingItem?'حفظ':'إضافة'}</button><button type="button" className="secondary" onClick={()=>setItemOpen(false)}>إلغاء</button>
  </form></Modal>

  {/* Supplier modal */}
  <Modal open={supOpen} onClose={()=>setSupOpen(false)} title={editingSup?'تعديل مورّد':'مورّد جديد'}><form className="form compact" onSubmit={saveSup}>
    <Field label="اسم المورّد" required><input required value={supForm.name} onChange={e=>setSupForm({...supForm,name:e.target.value})}/></Field>
    <Field label="الهاتف"><input value={supForm.phone} onChange={e=>setSupForm({...supForm,phone:e.target.value})}/></Field>
    <Field label="ملاحظات" wide><textarea value={supForm.notes} onChange={e=>setSupForm({...supForm,notes:e.target.value})}/></Field>
    <button>{editingSup?'حفظ':'إضافة'}</button><button type="button" className="secondary" onClick={()=>setSupOpen(false)}>إلغاء</button>
  </form></Modal>

  {/* Movement modal */}
  <Modal open={movOpen} onClose={()=>setMovOpen(false)} title={movType==='purchase'?'تسجيل شراء':movType==='consume'?'تسجيل صرف':'تسوية مخزون'}><form className="form compact" onSubmit={saveMov}>
    <Field label="الصنف" required><select required value={movForm.item_id} onChange={e=>setMovForm({...movForm,item_id:e.target.value})}><option value="">اختر الصنف</option>{items.map(i=><option key={i.id} value={i.id}>{i.name} — متاح {fmt(i.stock)} {i.unit}</option>)}</select></Field>
    {selItem&&movType==='consume'&&<div className="invMovHint">المتاح حالياً: <strong>{fmt(selItem.stock)} {selItem.unit}</strong></div>}
    <Field label={`الكمية${selItem?' ('+selItem.unit+')':''}`} required><input required type="number" min="0.01" step="0.01" value={movForm.quantity} onChange={e=>setMovForm({...movForm,quantity:e.target.value})}/></Field>
  ...
  {/* Import modal */}
  <Modal open={impOpen} onClose={()=>setImpOpen(false)} title="استيراد بيانات المخزن">
    <div className="invImport">
      <div className="invImportSteps">
        <p className="invImportHint">حمّل النموذج، املأ الشيتات (الأصناف / الموردون / المشتريات)، ثم ارفع الملف.</p>
        <button type="button" className="secondary invImportDl" onClick={downloadInvTemplate}><Download size={15}/>تحميل نموذج Excel</button>
        <input ref={impFileRef} type="file" accept=".xlsx,.xls" style={{display:'none'}} onChange={onImpFile}/>
        <button type="button" onClick={()=>impFileRef.current?.click()}><Upload size={15}/>{impFile?impFile.name:'اختيار ملف'}</button>
      </div>
      {impLoading&&<div className="invImportLoading"><Loader2 size={18} className="spin"/> جاري المعالجة...</div>}
      {impPreview&&<div className="invImportPreview">
        <div className="invImportCounts">
          <div className="invImportCount"><strong>{impPreview.counts.items}</strong><span>أصناف</span></div>
          <div className="invImportCount"><strong>{impPreview.counts.suppliers}</strong><span>موردون</span></div>
          <div className="invImportCount"><strong>{impPreview.counts.purchases}</strong><span>مشتريات</span></div>
        </div>
        {impPreview.errors.length>0
          ? <div className="invImportErrors"><AlertCircle size={13}/><div><strong>{impPreview.errors.length} خطأ يجب تصحيحه:</strong><ul>{impPreview.errors.slice(0,10).map((e,i)=><li key={i}>{e}</li>)}</ul></div></div>
          : <button type="button" className="invImportConfirm" disabled={impLoading} onClick={impConfirm}><Check size={15}/>تأكيد الاستيراد</button>}
      </div>}
      {impResult&&<div className="invImportResult">
        <CheckCircle2 size={18}/>
        <div>
          <strong>تم الاستيراد بنجاح</strong>
          <div className="invImportResultLine">أصناف جديدة: {impResult.items} · موردون: {impResult.suppliers} · مشتريات: {impResult.purchases} · أرصدة افتتاحية: {impResult.opening}</div>
          {impResult.skipped?.length>0&&<div className="invImportSkipped">تم تخطّي {impResult.skipped.length}: {impResult.skipped.slice(0,5).join(' · ')}{impResult.skipped.length>5?'...':''}</div>}
        </div>
      </div>}
    </div>
  </Modal>
